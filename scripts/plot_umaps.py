...
def make_pca(clusters):
    ...
    legend1 = ax.legend(*scatter.legend_elements(),
                    loc="lower right", title="Groups")
    ax.add_artist(legend1)
    plt.legend(loc="upper right")
    plt.savefig("PCAS/pca."+anchor_name+"."+chrom_in+".bins"+str(max_bins)+".genome.pos.png")
    plt.close()
...
filename="UMAPS/umap.cluster."+anchor_name+"."+chrom_in+".bins"+str(max_bins)+".neighbors"+str(n_neighbors)+".md"+str(md)+".nskips."+str(n_skips)+".genome.pos.maxbinstest.eps"+str(eps)
...
def plot_interactive(anchor_name, chrom, start_coord, end_coord, step, pancounts, paircounts, genes):
        ...
        fig.add_trace(go.Heatmap(
            z=paircounts, x=paircounts.columns, y=paircounts.index, colorscale='plasma_r',#colorscale=[[0, 'rgb(0,0,255)'], [1,'rgb(255,0,0)']],
