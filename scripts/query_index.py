...
def query_something(ele, chrom):
    if ele == "gene":
        ...
        start = 0
        stop = index.chrs.loc[anchor_name, chrom][
            "size"
        ]  # This will go through every kmer in the chromosome
        ...
        raw_locs = np.flatnonzero(
            (kmers[:, 0] == 1) & (kmers[:, 1] == 0) & (kmers[:, 2] == 1) & (kmers[:, 3] == 0)
        )
    else:
...
def find_gene_avgs(elements):
    ...
    if ele != "bit":
        for i, row in elements.iterrows():
            these_kmers = index.query_bitmap(
                anchor_name, row["chr"], row["start"], row["end"], 1
            ).sum(axis=1)
    ...
    with open(cpg_file, "r") as f:
        ...
        while line:
            ...
            if len(tmp) > 1:
                ...
                this_stop = int(tmp[3])
                this_kmers = index.query_bitmap(
                    anchor_name, this_chr, this_start, this_stop, 1
                ).sum(axis=1)
