...
class Index(Serializable):
    ...
    gff_anno_types: List[str] = field(
        default=None, help="GFF features of which to store locations, but not conservation scores"
    )
    ...
    genome_umap: UMAP = field(
        default_factory=lambda: UMAP(neighbors=4, dist=0, eps=1, samples=1, bin_size=100000)
    )

    chrom_umap: UMAP = field(
        default_factory=lambda: UMAP(neighbors=4, dist=0, eps=1, samples=1, bin_size=100000)
    )
...
class Genome:
    ...
    def init_read(self):
        ...
        self.total_paircounts = pd.read_csv(
            os.path.join(self.prefix, "total_paircounts.csv"), index_col="name"
        )
    ...
    def run_anchor(self, bitvecs, logfile=None):
        ...
        self.bitmaps = {
            s: bgzip.BGZipWriter(open(self.bitmap_gz_fname(s), "wb")) for s in self.steps
        }
        bin_occs = dict()
    ...
    def write_umaps(self):
        ...
        for chrom in self.chrs.index:
            ...
            paircounts = self.index.bitmap_to_paircount_bins(
            ...
            genome_paircounts[chrom] = self.index.bitmap_to_paircount_bins(
                bitmap, self.index.genome_umap.bin_size
            ).T.fillna(0)

        self.chrom_umaps = pd.concat(chrom_umaps).set_index("chrom")
        self.chrom_umaps.to_csv(self.chrom_umaps_filename)

        self.genome_umap = self.run_umap(
            pd.concat(genome_paircounts, names=["chrom", "start"]), self.index.genome_umap
        )
    ...
    def run_umap(self, paircounts, args):
        ...
        if embedding is not None:
            ...
            out = pd.DataFrame(
                embedding, index=paircounts.index, columns=["umap1", "umap2"]
            ).reset_index()
            out["cluster"] = clusters
        else:
            out = pd.DataFrame(
                {"umap1": 0, "umap2": 0, "cluster": 0}, index=paircounts.index
            ).reset_index()
    ...
    def bin_bitsum(self, bitsum):
        ...
        return pd.DataFrame(
            [
                pd.Series(bitsum[st:en]).value_counts().reindex(self.bitsum_index, fill_value=0)
                for st, en in coords
            ],
            index=coords,
        ).astype(int)
