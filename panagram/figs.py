    ...
    for i, (c, freqs) in enumerate(totals.iterrows()):
        ...
        fig.add_trace(
            go.Bar(
                x=perc.index,
                y=perc,
                marker_color=genome_colors(index),
                showlegend=False,
            ),
            row=i + 1,
            col=1,
...
def make_all_genome_dend(index):
    ...
    fig = ff.create_dendrogram(
        df, colorscale=branch_colors, labels=df.columns, orientation="bottom"
    )  # , orientation='bottom'
    fig.update_layout(
    ...
    fig.update_layout(
        {
            "autosize": True,  #'width':1500, 'height':1500,
            "showlegend": False,
            "hovermode": "closest",
            "paper_bgcolor": "rgba(0,0,0,0)",
...
def read_pangenome_comp(index):
    ...
    for i, g in enumerate(index.genome_names):
        fig.add_trace(
            go.Bar(
                y=index.genome_names,
                x=index.bitfreq_totals[i + 1],
                name=i + 1,  # orientation='h',
