...
def view(params):
    ...
    whole_genome_hists_fig = figs.read_genome_comp(
        index, anchor_name
    )  # TODO update on anchor change
    ...
    ANCHOR_TAB = [
        ...
        html.Div(
            className="w3-third",
            children=[dcc.Graph(id="genes_per_chr", config=config)],
        ),
        html.Div(
            className="w3-third",
            children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.I(
                                "",
                                style={"font-size": 30},
                                id="annotation_tab_clickdata_Name",
                            ),
                            html.Br(),
                            html.I(
                                "",
                                style={"font-size": 30},
                                id="annotation_tab_clickdata_X",
                            ),
                            html.Br(),
                            html.I(
                                "",
                                style={"font-size": 30},
                                id="annotation_tab_clickdata_Y",
                            ),
                            html.Br(),
                            html.I(
                                "",
                                style={"font-size": 30},
                                id="annotation_tab_clickdata_Color",
                            ),
                            html.Br(),
                            html.P(
                                "",
    ...
    app.layout = html.Div(
        [
            ...
            html.H1(
                id="H1",
                children="Panagram",
                style={"textAlign": "center", "font-size": 64, "padding": "1%"},
            ),
            html.Div(
                children=[
                    html.Div(
                        children=[
                            ...
                            # html.Br(),
                            html.I(
                                "Step size: " + str(index.lowres_step),
                                style={
                                    "display": "inline-block",
                                },
                                id="step_size_out",
                    ...
                    html.Div(
                        children=[
                            ...
                            dcc.Input(
                                id="Chrs_Info", debounce=True
                            ),  # , placeholder=str(x_start_init) + "-" + str(x_stop_init)+ "          "
                            html.Br(),
                            html.I(
                                id="regional_genes"
                            ),  # "Genes in this region: " + str(local_genes),
                            html.Br(),
                            ...
                            ),  # pre_bed_info[anchor_name], chrs + ":" + str(x_start_init) + "-" + str(x_stop_init) ,
                        ],
                        style={
                            "display": "inline-block",
                            "padding-left": "10%",
    ...
    def get_clade_lines(
        ...
        if orientation == "horizontal":
            branch_line.update(x0=start_coord, y0=y_curr, x1=x_curr, y1=y_curr)
    ...
    def create_tree(bitmap):
        ...
        fig = make_subplots(
            ...
            cols=1,
            specs=[
                [{"type": "scatter", "colspan": 1}]
            ],  # , None, {"type": "bar"} ]], #, {"type": "pie"} , {"type": "pie"} ]],
    ...
    def plot_chr_whole(start_coord, end_coord, anchor_name, this_chr, genes):
        ...
        chr_fig.update_yaxes(
            title_text="Genes", range=[0.5, 1.5], showticklabels=False, row=3, col=1
        )
    ...
    def plot_whole_genome(anchor_name):
        ...
        wg_fig = make_subplots(
            rows=(3 * index[anchor_name].chr_count),
            cols=1,
    ...
    def nav_callback(
        ...
        # Chromosome main plot, triggers CHROMOSOME
            ...
            if (
                ...
                and "xaxis.range[0]" in chrtab_primary_relayout.keys()
            ):
                start_coord = int(chrtab_primary_relayout["xaxis.range[0]"])
                end_coord = int(chrtab_primary_relayout["xaxis.range[1]"])
    ...
    def update_all_figs(chr_num, chrom, anchor_name, redo_wg, start_coord, end_coord, n_skips):
        ...
        bar_sum_regional = pan.sum(
            axis=1
        )  # bitmap_counts.value_counts().reindex(index.bitsum_index, fill_value=0)
